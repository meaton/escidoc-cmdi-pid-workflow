...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:cmd="http://www.clarin.eu/cmd/">
    ...
    <xsl:template match="cmd:Components">
        <cmd:Components>
          <cmd:teiHeader>
	    <cmd:type><xsl:value-of select="cmd:olac/cmd:type"/></cmd:type>
            <cmd:fileDesc>
              <cmd:titleStmt>
              ...
              <cmd:profileDesc>
                  ...
                  <cmd:langUsage>
                      ...
                      <cmd:language ident="{$lang}"> <!-- hvad med sprogkode? -->
                          <xsl:if test="($lang = 'da' or $lang = 'dan')">da</xsl:if>
                          <xsl:if test="$lang = 'de'">de</xsl:if>
                          <xsl:if test="$lang = 'en'">en</xsl:if>  
