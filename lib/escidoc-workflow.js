var http = require('http'), EventEmitter = require('events').EventEmitter, util = require('util'), xml = require('libxmljs'), config = require('./config.js');
...
var eSciDocWorkflow = function() {
	...
	this.createMdRecord = function(escidocID, itemDoc, mdrecord, pid) {
		...
		if(md_scaf != null) {
		    ...
		    var child = md_scaf.child(md_scaf.childNodes().length-1)
		    child.attr({'name': 'cmdi'}).addChild(mdrecord.root());
...
var parseContentComponents = function(escidocID, Components, callback) {
	...
	if(resources_count > 0) {
		...
		resources[componentID] = {parentID: escidocID, componentID: componentID, checksum: checksum, mimeType: mimeType};
...
var ir_options = function(params) {
		...
		headers: {'Content-Type': 'application/xml', 'Cookie': 'escidocCookie='+params.handle, 'Cache-Control':'no-cache, private, no-store, must-revalidate, max-stale=0, post-check=0, pre-check=0'},
...
var ir = function(escidocID, method, data, lastModificationDate, callback) { 
    	var req = http.request(ir_options({id: escidocID, method: method, lastModificationDate: lastModificationDate, handle: config.escidoc_handle}), function(res) {
	        var d = "";
