var https = require('https'), util = require('util'), EventEmitter = require('events').EventEmitter, config = require('./config.js'), xml = require('libxmljs');
...
var PidWorkflow = function() {
	...
	var escidocID = data[1], type = data[0], objID = escidocID, version = 0;
	...
	var url =  (url != null || url != undefined) 
		? url 
		: "http://" + config.pid_url_host + config.pid_url_path + '/' + stripPrefix(data[2]) + '/' + stripPrefix(data[1]) + '?v=' + version;
	
	var request = https.request({host: config.pidmanager_host, path: config.pidmanager_path, method: 'POST', auth: config.pidmanager_auth_user + ':' + config.pidmanager_auth_pass}, function(res) {
	    res.on('error', function(err) {
		console.log('Error occurred assigning PID for ' + escidocID + ': ' + err);
	    });
	    res.on('data', function(chunk) {
		// get PID value 
		var pid, pid_data = {escidocID: objID, val:val, type: type, url: url, lastdatetime: lastdatetime};
	...
	request.on('error', function(e) { job.done(e); });
	request.write(createPostData(escidocID, url, val));
	...
	var pidDocument = xml.parseXmlString(data), pid;
