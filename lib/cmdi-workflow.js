...
var CMDIWorkflow = function() {
  ...
  this.getOriginalMD = function(escidocID, callback) {
    return retrieveMDRecord(updater_options({
      ...
      callback(xml.parseXml(d))
    });
  };

  this.transform = function(md, params, callback) {
    xslt.parseFile(config.xslt_filepath, function(err, stylesheetObj) {
      if (err) throw err;
      var stylesheet = stylesheetObj; //xslt.parse(data);
      var document = (typeof md == "string" || md instanceof String) ? xml.parseXml(md) : md;

      if (params == null) params = config.xslt_params;
      stylesheet.apply(document, params, function(err, result) {
          if(!err) callback(result);
          else console.error('xslt error: ' + err);
      });
    });
  };

  this.removeNamespacesFromXML = function(md, callback) {
    xslt.parseFile(config.xslt_removeNamespace, function(err, stylesheetObj) {
      if (err) throw err;
  ...
  this.buildCMDIHeader = function(escidocID, md, md_pid, callback) {
    ...
    var mdDocument = xml.parseXml(md);
    ...
    self.once('validate-' + escidocID, function(newMdDocument) {
      ...
      fs.writeFile(xmlFilePath, newMdDocument, function(err) {
        if (err) console.error('error saving Metadata file: ' + xmlFilePath);
          ...
          var Module = {
            arguments: ["--path", "./data/schemas", "--nonet", "--noout", "--schema", config.xsd_filepath, xmlFilePath]
          };
...
*/
  ...
  this.createTEIKeywords = function(item, md, callback) {
    var keywords = item.root().find(config.target_tei_keywords_xpath, config.ns_obj);
    ...
    if (keywords != null && keywords.length > 0 && textClass_cmdi != null) {
      for(var i=0; i < keywords.length; i++) {
        var keywords_elem = null;
        var nReg = /\D+/g;
        var terms = keywords[i].find('t:term', config.ns_obj);
        if(terms.length > 0 && nReg.test(terms[0].text())) { // check valid term values
          if (keywords_cmdi == null) {
            keywords_elem = new xml.Element(md, "keywords");
            var schemeVal = keywords[i].attr("scheme").value();
            if(schemeVal != null && schemeVal != 'nil')
              keywords_elem.attr({
                scheme: schemeVal
              }); //TODO Show scheme value be left unmodified?
            textClass_cmdi.addChild(keywords_elem);
          } else if (keywords_cmdi.attr("scheme") == null) {
            keywords_cmdi.attr({
              scheme: keywords[i].attr("scheme").value()
            });
            keywords_elem = keywords_cmdi;
          }

          if(keywords_elem != null) {
            console.log("Adding keywords to CMDI teiHeader component:" + keywords_cmdi);
            // create cmd:list attr type:"simple" elem
            var list = keywords_elem.addChild(new xml.Element(md, "list")).get("list").attr({
              type: "simple"
            });
  ...
  this.createTEIAnnotation = function(item, md, callback) {
    var promises = [];
    var appInfo = new xml.Element(md, "appInfo");
    ...
    if (applAnnos != null && applAnnos.length > 0 && encodingDesc != null) {
      ...
      for (var i = 0; i < applAnnos.length; i++) {
        ...
        if(applAnno.attr('ident').value() != 'nil') {
          ...
          var promise = new Q.Promise(function(resolve, reject) {
            self.removeNamespacesFromXML(applAnno.toString(), function(xml_data) {
              var newApplAnno = xml.parseXml(xml_data).root();
      ...
      Q.all(promises).then(function() {
        if(appInfo.find('application').length > 0)
          encodingDesc.addChild(appInfo); // add non-empty appInfo element
    ...
    if (revisionDesc != null) {
      new Q.Promise(function(resolve, reject) {
        self.removeNamespacesFromXML(revisionDesc.toString(), function(xml_data) {
          var newRevisionDesc = xml.parseXml(xml_data).root();
          ...
          resolve(newRevisionDesc);
        });
      }).then(function(newRevisionDesc) {
