...
*/
  ...
  this.createTEIAnnotation = function(item, md, callback) {
    ...
    if (applAnno != null && applAnnos.length > 0 && encodingDesc != null) {
      var appInfo = encodingDesc.addChild(new xml.Element(md, "appInfo")).get('appInfo'); // add empty appInfo element
      console.log('Found application: ' + newApplAnnos.length);

      for (var i = 0; i < newApplAnnos.length; i++) {
        var applAnno = newApplAnnos[i];
  ...
  this.createTEIRevisionDesc = function(item, md, callback) {
    ...
    var revisionDesc_cmdi = md.get(config.target_cmdi_revision_desc_xpath);
