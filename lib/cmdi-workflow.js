var http = require('http'), config = require('./config.js'), EventEmitter = require('events').EventEmitter, util = require('util'), xslt = require("libxsltjs"), xml = require('libxmljs'), fs = require('fs'), spawn = require('child_process').spawn, path = require('path');
...
var CMDIWorkflow = function() {
	...
	this.getOriginalMD = function(escidocID, callback) {
		return retrieveMDRecord(updater_options({id: escidocID, name: config.target_mdrecord_name, target: config.escidoc_host, handle: config.escidoc_handle}), function(d) { 	if(util.isError(d)) return self.emit('error', d); callback(xml.parseXmlString(d)) });
	}
	...
	this.buildCMDIHeader = function(escidocID, md, md_pid, callback) {
	    ...
	    mdDocument.root().attr({xmlns: config.clarin_schema_url, 'xsi:schemaLocation': config.clarin_schema_url + ' ' + cmdSchema}); 
	    ...
	    */
		...
		fs.writeFile(xmlFilePath, newMdDocument, function(err) {
		   if(err) console.error('error saving Metadata file: ' + xmlFilePath);
		      ...
		      //TODO spawn xmllint process
		      var Module = {arguments: ["--noout", "--schema", config.xsd_filepath, xmlFilePath]};
		      var xmllint = spawn('xmllint', Module.arguments);
			...
			if(!config.debug) fs.unlink(xmlFilePath, function(err) { if(err) console.error('cannot remove file: ' + xmlFilePath); else console.log('removed test file: ' + xmlFilePath);
		        });
		      });
		   }
		});
...
*/
	this.addResource = function(md, resource, resources_len) {
		...
		if((resourceElem.childNodes().length) <= 0)
		    rProxy = resourceElem.addChild(new xml.Element(md, 'ResourceProxy')).child(0).attr({id: '_'+componentID});
		else 
		    rProxy = resourceElem.child(0).addPrevSibling(new xml.Element(md, 'ResourceProxy')).attr({id: '_'+componentID});
		var rType = rProxy.addChild(new xml.Element(md, 'ResourceType', 'Resource')).child(0).attr({mimetype: new String(resource.mimeType)});
	...
	this.updateTEIFileDesc = function(item, md, callback) {
	    var title = null, name_ref = null, note_resp = null, biblStruct = null, idno_uri = null, publStmt = null;
		...
		if((note_resp = md.get(config.target_cmdi_notesStmt_note_attr)) != null && note_resp.attr("resp") == null)
		    note_resp.attr({resp: notes.attr("resp").value()});
		...
		if((idno_uri = md.get(config.target_cmdi_idno_uri)) == null) {
		    ...
		    if(biblStruct != null && biblStruct.childNodes().length >= 1)
		       biblStruct.child(0).addPrevSibling(new xml.Element(md, "idno", idno.text()).attr({type:"uri"}));
	   ...
	   if((context = item.get(config.target_context_model_prop, config.ns_obj)) != null) {
		...
		if((publStmt = md.get(config.target_cmdi_publStmt)) != null)
    		    publStmt.addChild(new xml.Element(md, "availability").attr({status:(context == 'aca') ? "restricted" : "free"}).addChild(new xml.Element(md, "ab").attr({type:(context == 'aca') ? "academic" : "public"})));
	...
	this.createTEIKeywords = function(item, md, callback) {
	    ...
	    if(keywords != null && textClass_cmdi != null) {
		    ...
		    keywords_cmdi = textClass_cmdi.addChild(new xml.Element(md, "keywords")).get("keywords").attr({scheme:keywords.attr("scheme").value()}); //TODO Show scheme value be left unmodified?
	        else if(keywords_cmdi.attr("scheme") == null) 
		    keywords_cmdi.attr({scheme:keywords.attr("scheme").value()});

	        console.log("Adding keywords to CMDI teiHeader component:" + keywords_cmdi);

		// create cmd:list attr type:"simple" elem
		var list = keywords_cmdi.addChild(new xml.Element(md, "list")).get("list").attr({type: "simple"});
	...
	this.createTEIAnnotation = function(item, md, callback) {
	    var applAnno = item.root().get(config.target_tei_anno_appl_xpath, config.ns_obj);
	    var appInfo = md.get(config.target_cmdi_appInfo_xpath);

	    if(applAnno != null && appInfo != null) { 
		...
		self.removeNamespacesFromXML(applAnno.toString(), function(xml_data) {
		    console.log('child: ' + xml_data);
	 	    var newApplAnno = xml.parseXmlString(xml_data);

		    appInfo.addChild(newApplAnno.root());
		    callback(item, md); 
		});
	    } else {
		console.log('No application element found.');
	    	callback(item, md);
	    }
	}
