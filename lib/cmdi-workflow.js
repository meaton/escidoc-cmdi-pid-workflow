...
*/
  this.addResource = function(md, resource, resources_len, resource_type) {
    ...
    if (resourceElem != null) {
      ...
      var rType = rProxy.addChild(new xml.Element(md, 'ResourceType', resourceType)).child(0).attr({
      ...
      if (resourceElem.childNodes().length == parseInt(resources_len)) {
        ...
        var versPID = get('//MdSelfLink').text(); // version PID item/collection
        addResource(md, { pid: versPID,
                          componentID: resource.parentID,
                          parentID: resource.parentID,
                          mimeType: 'text/html' },
                    resources_len, 'LandingPage');
      else if(resourceElem.childNodes().length >= parseInt(resource_len)) {
