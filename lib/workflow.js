var fs = require('fs'), path = require('path'), byline = require('byline'), util = require('util'), utile = require('utile'), _u = require('underscore'), kue = require('kue'), xml = require('libxmljs'), config = require('./config.js');
// job queue
var jobs = kue.createQueue(), Job = kue.Job, currentJob = null, log_file = null;
...
pidWorkflow.on('createdItemPID', function(escidocID, pid, url) { // fetch md-record TEI/CMD	
	...
	var escidoc_job = jobs.create('escidoc workflow', {title: 'creating new CMDI md record for ' + escidocID, escidocID: escidocID, pid:pid, url:url});
...
var createLoggingFile = function(callback) {
	...
	fs.readdir(dir, function(err, files) {
		...
		console.log('file found: ' + name);
	  	if(name.indexOf('completed-jobs.log')) file_count++
	    });
...
var parseComponents = function(resources, itemDoc, cmdi, url, lastdatetime) {
	 ...
	 _u.each(resources, function(val, key, list) {
	    var componentID = val.componentID, parentID = val.parentID, mimeType = val.mimeType;
	    ...
	    escidocWorkflow.once('content-assigned-pid-' + componentID, function(componentID, pid) {
	        ...
	        cmdiWorkflow.emit('add-resource-'+componentID, {parentID: parentID, componentID: componentID, pid: pid, mimeType: mimeType});
	    });
	
	    escidocWorkflow.once('completed-component-' + componentID, function(lastdatetime) {
		if(lastdatetime != null) itemDoc.root().attr({'last-modification-date': lastdatetime});
	        var nextComponent = (list.length >= key+1) ? list[key+1] : null;
	        if(nextComponent != null)
	            createComponentPIDJob(nextComponent, url, lastdatetime);     
	    });
        });
    
    var firstComponent = resources[0];
    var url = url.substring(0, url.lastIndexOf("?v=")).concat("/" + firstComponent.componentID.replace(/\D/g, ""));

    createComponentPIDJob(firstComponent, url, lastdatetime);
}

var createComponentPIDJob = function(component, url, lastdatetime) {
    var job = jobs.create('pid workflow', { title: 'creating component PID for ' + component.componentID, component: component, url: url, lastdatetime: lastdatetime });
...
var outStream = fs.createWriteStream(path.join(config.log_dir, 'processed.txt'));
...
members.on('data', function(data) {
   if(data != null) {
	...
	var job = jobs.create('pid workflow', { title: 'creating version PID for ' + data[1], escidocID: data[1], contentModelID: data[2], version: data[4]});
...
members.on('end', function() {
   ...
   jobs.process('pid workflow', function(job, done) {
	...
	if(job.data.component != null) 
	    pidWorkflow.generatePid(["component", job.data.component.componentID], job.data.url, job.data.component, job.data.lastdatetime, {id: job.id, done: done});
	else
	    pidWorkflow.generatePid(['item', job.data.escidocID, job.data.contentModelID, '', job.data.version], null, null, null, {id: job.id, done: done});
   });

   jobs.process('escidoc workflow', function(job, done) {
	    currentJob = job;
	    console.log('processing escidoc workflow task... ' + job.data.escidocID);
	    var current = 1, complete = 11;
		...
		job.log('Fetching contents...');
			...
			job.progress(current++, complete);
		   	...
		   	if(md.root().text().replace(/\s/g, "") == mdrecord.text().replace(/\s/g, ""))
			    ...
			    if(md.root().attr("CMDVersion") == null) md.root().attr({"CMDVersion": "1.1"});
				...
				cmdiWorkflow.updateTEIFileDesc(itemDoc, cmdi, function(itemDoc, cmdi) {
					...
					cmdiWorkflow.createTEIAnnotation(itemDoc, cmdi, function(itemDoc, cmdi) {	
					    job.progress(current++, complete);
			                    parseComponents(resources, itemDoc, cmdi, job.data.url, lastdatetime);
					});
				    });
				});
		            });
