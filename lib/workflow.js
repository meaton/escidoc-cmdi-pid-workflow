...
var parseComponents = function(resources, itemDoc, cmdi, url, lastdatetime) {
  console.log('Parsing eSciDoc Components: ' + resources.length);
  var url_o = { url : url };
  ...
  _u.each(resources, function(val, key, list) {
    ...
    escidocWorkflow.once('completed-component-' + componentID, function(lastdatetime) {
      ...
      if (nextComponent != null)
        createComponentPIDJob(nextComponent, utile.clone(url_o).url, lastdatetime);
    });
  });

  var firstComponent = resources[0];
  var url_r_1 = url_r.url.substring(0, url_r.url.lastIndexOf("?v=")).concat("/" + firstComponent.componentID.replace(/\D/g, ""));
...
var members = new LineStream();

// MemberStream events
members.on('data', function(line) {
  if (line != null) {
    var data = line.toString('utf8').split('%');
...
module.exports = inStream.pipe(members);
